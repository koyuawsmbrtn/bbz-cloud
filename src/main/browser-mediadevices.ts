...
const { h, render } = require('react-dom');
...
const desktopCapturer = {
  ...
  getSources: (opts) => ipcRenderer.send('desktopCapturerGetSources', opts),
};
  ...
  return html`
    ...
    <div class="${ROOT_CLASS}__overlay" onclick=${cancel}>
      <div class="${ROOT_CLASS}__sources">
        ...
        ${sources !== null &&
        sources.map(
          (source: any) => html` <${Source} resolve=${resolve} ...${source} /> `
        )}
      </div>
    </div>
  `;
...
const getDisplayMedia = async (resolve: any, reject: any) => {
  currentRoot = render(
